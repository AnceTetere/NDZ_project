splittingThrees <- function(x3) {
  ...
  #    x3$zinkod[1] %in% c("92", "41") && x3$zinkod[3] %in% c("92", "41") && x3$PS_code[1] == '_________' && x3$NM_code[1] == '_________'){
  ...
  if (all(x3$sak_beidz == c("1", "2", "2")) && all(x3$zinkod != '26') && 
     ...
     x_vieninieki <- rbind(x_vieninieki, x3[3, ])    
  } else if (all(x3$sak_beidz == c("2", "1", "2")) && diff(x3$NDZ_sanemsanas_datums[1:2]) != 0) {
