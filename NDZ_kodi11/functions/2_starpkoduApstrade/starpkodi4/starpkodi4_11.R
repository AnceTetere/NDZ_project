          ...
          if (t$zinkod[3] %in% c("11", "16", "61")) {
            if (t$zinkod[4] == "50") {
              if (all(diff(t$ZDN_sanemsanas_datums) != 0)) {
                yt <- y[v, ]
                yt$dienas <- sum(as.numeric(diff(t$ZDN_sanemsanas_datums[1:2])) + 1,  
                             as.numeric(diff(t$ZDN_sanemsanas_datums[3:4])))
            
              } else if (all(sapply(seq(1,4,by=2), function(i) diff(t$ZDN_sanemsanas_datums[i:(i+1)]) == 0)) &&
...
#    yt <- y[v, ]
#    yt$dienas <- as.numeric(difftime(t$ZDN_sanemsanas_datums[4], t$ZDN_sanemsanas_datums[3], units = "days"))
...
#    yt <- y[v, ] 
#    yt$dienas <- as.numeric(difftime(t$ZDN_sanemsanas_datums[4], t$ZDN_sanemsanas_datums[3], units = "days")) + 1 
...
#    yt <- y[v, ] 
#    yt$dienas <- as.numeric(difftime(t$last_date[4], t$ZDN_sanemsanas_datums[4], units = "days")) + 1 
#   } else if (t$zinkod[2] == "92" && t$zinkod[3] == "40" && t$zinkod[4] == "41" && 
#              all(diff(t$ZDN_sanemsanas_datums) == 0) && t$PS_code[1] == '___________' && t$NM_code[1] == '___________') {
#     yt <- y[v, ]
#     yt$dienas <- 0
...
#     yt <- y[v:(v+1), ]
#     yt <- yt[yt$zinkod == "11", ]
#   } else if (t$zinkod[2] == "40" && t$zinkod[3] == "41" && t$zinkod[4] == "40" && all(diff(t$ZDN_sanemsanas_datums) != 0)) {
#     yt <- y[v, ]
#     yt$dienas <- sum(sapply(seq(1,4,by=2), function(i) as.numeric(difftime(t$ZDN_sanemsanas_datums[i+1], t$ZDN_sanemsanas_datums[i], units = "days"))))
#   } else if (t$zinkod[2] == "40" && t$zinkod[3] == "25" && t$zinkod[4] == "41" && 
#              diff(t$ZDN_sanemsanas_datums[2:3]) != 0 && all(sapply(seq(1,4, by=2), function(i) diff(t$ZDN_sanemsanas_datums[i:(i+1)]) == 0))) {
#     yt <- y[v, ]
#     yt$dienas <- 0
