starpkodi8 <- function(y2, t, prev, v) {
    ...
    rm(days1, days2, days3, days4)
             ...
             t$PS_code[1] == '_____________' && t$NM_code[1] == '____________') {
    ...
    days2 <- as.numeric(difftime(t$beidz[4], t$sak[3], units = "days"))
    days3 <- as.numeric(difftime(t$beidz[6], t$sak[5], units = "days"))
    days4 <- as.numeric(difftime(t$beidz[8], t$sak[7], units = "days")) + 1 
    ...
    for (d in seq(2,6,by=2)) {days <- days + as.numeric(difftime(t$beidz[d+1], t$sak[d], units = "days"))}
    days <- days + as.numeric(difftime(t$last_date[8], t$sak[8], units = "days")) + 1
