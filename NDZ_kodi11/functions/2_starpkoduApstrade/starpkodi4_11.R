starpkodi4_11 <- function(y2, t, prev, v) {
  if (t$zinkod[2] == "50" && t$NDZ_sanemsanas_datums[1] == t$NDZ_sanemsanas_datums[2] && 
  ...
  } else if (t$zinkod[2] == "50" && t$zinkod[3] == "22" && t$zinkod[4] == "51" && 
             all(diff(t$NDZ_sanemsanas_datums[1:3]) != 0) && t$NDZ_sanemsanas_datums[3] == t$NDZ_sanemsanas_datums[4]) {
    yt <- y2[v, ]
    yt$dienas <- as.numeric(difftime(t$beidz[2], t$sak[1], units = "days"))
    ...
    yt <- y2[v, ] 
    yt$dienas <- as.numeric(difftime(t$beidz[4], t$sak[3], units = "days")) + 1 #jo darbs
    ...
    yt$dienas <- sum(days1, days2)
    rm(days1, days2)
  } else if (t$zinkod[2] == "50" && t$zinkod[3] == "50" && t$zinkod[4] == "51" && 
    ...
    days2 <- as.numeric(difftime(t$last_date[4], t$sak_darbu[4], units = "days")) + 1			 
