...
#!!! lieku šādās stingrās kombinācijās, jo ar %in% tas var miksēt, un es neesmu pārliecināta, ka visas tās kombinācijas izpildās.
if ((t$zinkod[1] == "11" && t$zinkod[2]  %in% c("40", "50", "53")) ||
  ...
  yt <- y2[v:(v+1),] 
  yt <- yt[yt$zinkod == "53", ]
} else if (t$zinkod[1] == "25" && t$zinkod[2] == "91" && t$PS_code[1] == '________' && t$NM_code[1] == '_________') {
  yt <- y2[v:(v+1),] 
  yt <- yt[yt$zinkod == "11", ]
  ...
  yt <- y2[v, ] 
} else if (t$zinkod[1] == "11" && t$zinkod[2] %in% c("51", "92")) {
  yt <- y2[v, ]
  yt$dienas <- as.numeric(difftime(t$last_date[2], t$sak[1], units = "days"))
} else if (t$zinkod[1] == "50" && t$zinkod[2] == "53" && t$PS_code[1] == '_________' && t$NM_code[1] == '______________') {
  yt <- y2[v:(v+1), ]
  yt <- yt[yt$zinkod == "50", ]
