...
#  #  yt <- y[v, ]
#  #  yt$dd <- as.numeric(difftime(t$sak_beidz[1], prev, units = "days"))
#  #} 
...
#      yt <- y2[v, ]
#      yt$dd <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")), as.numeric(diff(t$NDZ_sanemsanas_datums[2:3]))) 
...
#    yt <- y2[v, ]
#    yt$dd <- as.numeric(difftime(t$beidz_darbu[1], prev, units = "days"))
