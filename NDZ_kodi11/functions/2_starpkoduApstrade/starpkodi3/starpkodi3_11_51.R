starpkodi3_11_51 <- function(y2, t, prev, v) {
  ...
  if (t$zinkod[2] == "51" && t$zinkod[3] == "51" && diff(t$NDZ_sanemsanas_datums[2:3]) != 0 && diff(t$NDZ_sanemsanas_datums[1:2]) == 0 && 
    ...
    yt$dienas <- as.numeric(difftime(t$last_date[3], t$sak[3], units = "days")) + 1
             ...
             t$PS_code[1] == '_____________' && t$NM_code[1] == '_____________') {
    yt <- y2[v, ]
    yt$dienas <- as.numeric(difftime(t$beidz_darbu[3], t$sak[1], units = "days"))
  } else if (t$zinkod[2] == "51" && t$zinkod[3] == "51" && t$PS_code[1] == '_____________' && t$NM_code[1] == '_____________') {
    yt <- y2[v, ]
    yt$dienas <- as.numeric(difftime(t$last_date[3], t$sak[3], units = "days")) + 1
