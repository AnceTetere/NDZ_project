starpkodi3 <- function(y2, t, prev) {
  if (t$zinkod[1] == "50" && t$zinkod[2] == "21" && t$zinkod[3] == "51" && t$NDZ_sanemsanas_datums[2] == t$NDZ_sanemsanas_datums[3]){
    ...
    days <- as.numeric(difftime(as.Date(t$beidz[1]), prev, units = "days")) - 1 #mīnus 1, jo 10-tajā viņš jau ir atvaļinājumā
    ...
    yt$dienas <- days
             ...
             t$zinkod[3] == "81" && t$NDZ_sanemsanas_datums[1] != t$NDZ_sanemsanas_datums[2] &&
             t$NDZ_sanemsanas_datums[2] == t$NDZ_sanemsanas_datums[3]) {
    #Indivīdu atlaiž no darba un tad pieņem darbā kā fizisku personu ar diviem kodiem "11" un "81".
    days1 <- as.numeric(difftime(as.Date(t$beidz[1]), prev, units = "days"))
    ...
    yt$dienas <- days
             ...
             t$zinkod[3] == "91" && all(!diff(t$NDZ_sanemsanas_datums) == 0)) {
    days1 <- as.numeric(difftime(as.Date(t$beidz[1]), prev, units = "days")) -1 #jo atvaļinājums
    days2 <- as.numeric(difftime(as.Date(t$beidz[3]), as.Date(t$sak[2]), units = "days")) + 1 #jo darbs
    days <- days1 + days2
    rm(days1, days2)
    ...
    yt$dienas <- as.numeric(difftime(as.Date(t$beidz[2]), as.Date(t$sak[1]), units = "days"))
            ...
            t$zinkod[3] == "11" && all(!diff(t$NDZ_sanemsanas_datums[2:3]) == 0) &&
