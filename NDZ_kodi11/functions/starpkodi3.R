starpkodi3 <- function(y2, t, prev) {
  if (t$zinkod[1] == "50" && t$zinkod[2] == "21" && t$zinkod[3] == "51" && t$NDZ_sanemsanas_datums[2] == t$NDZ_sanemsanas_datums[3]){
    ...
    days1 <- as.numeric(difftime(as.Date(t$beidz_darbu[2]), as.Date(t$sak_darbu[1]), units = "days")) #mīnus 1, jo 10-tajā viņš jau ir atvaļinājumā
    days2 <- as.numeric(difftime(as.Date(t$last_date[3]), as.Date(t$sak_darbu[3]), units = "days")) + 1 #mīnus 1, jo 10-tajā viņš jau ir atvaļinājumā
    days <- days1 + days2 
    rm(days1, days2)
