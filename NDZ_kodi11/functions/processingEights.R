processingEights <- function(x, o){
  x <- x %>% arrange(PS_code, DN_code, nmrkod, NDZ_sanemsanas_datums)
  ...
  for (r in seq(1, nrow(x), by = 8)) {
    x8 <- x[r:(r+7),] %>% arrange(PS_code, DN_code, nmrkod, NDZ_sanemsanas_datums)
    
    if (x8$PS_code[1] == '_________' && x8$nmrkod[1] == '_________') {
      ...
      x8_uzSeptini <- rbind(x8_uzSeptini, x8[-1, ])
               ...
               x8$PS_code[1] == '_________' && x8$nmrkod[1] == '_________') {
...
if(nrow(x8_uzVieniniekiem) > 0) {
  x8_uzVieniniekiem %>% arrange(PS_code, nmrkod, NDZ_sanemsanas_datums) %>% processingOnes(o) %>% sendTo_tempNDZ()
...
if (nrow(x8_uzDivniekiem) > 0) {
  ...
  x8_uzDivniekiem %>% arrange(PS_code, nmrkod, NDZ_sanemsanas_datums) %>% processingTwoes(o)
...
if (nrow(x8_uzCetriniekiem) > 0) {
  ...
  x8_uzCetriniekiem %>% arrange(PS_code, nmrkod, NDZ_sanemsanas_datums) %>% processingFours(o)
...
if (nrow(x8_uzSesi) > 0) {
  ...
  x8_uzSesi %>% arrange(PS_code, nmrkod, NDZ_sanemsanas_datums) %>% processingSixes(o)
...
if (nrow(x8_uzSeptini) > 0) {
  ...
  x8_uzSeptini %>% arrange(PS_code, nmrkod, NDZ_sanemsanas_datums) %>% processingSeven(o)
