processingEights <- function(x, o){
  ...
  for (r in seq(1, nrow(x), by = 8)) {
    ...
    if (sum(x8$sak_beidz == "1") == 4) {
        result <- processingEights_s4(x8)
    } else if (x8$sak_beidz[1] == "2" && x8$sak_beidz[2] == "1" && x8$NDZ_sanemsanas_datums[1] != x8$NDZ_sanemsanas_datums[2]) {
    ...
    } else if (x8$sak_beidz[1] == "1" && x8$sak_beidz[2] == "2" && x8$sak_beidz[3] == "2" && x8$sak_beidz[4] == "1" && 
    ...
    } else if (all(x8$sak_beidz[c(1, 3:4, 6:8)] == "1") && 
               all(diff(x8$NDZ_sanemsanas_datums[1:7]) != 0) && all(diff(x8$NDZ_sanemsanas_datums[7:8]) == 0) &&
               x8$PS_code[1] == '_________' && x8$NM_code[1] == '_____________') {
      x8_uzVieniniekiem <- rbind(x8_uzVieniniekiem, x8[1,])
    } else if (all(x8$sak_beidz[c(1, 3, 5, 7, 8)] == "1") && all(diff(x8$NDZ_sanemsanas_datums) != 0)){
...
if (nrow(x8_uzCetriniekiem) > 0) {
  ...
  processingFours(x8_uzCetriniekiem, "4")
...
if (nrow(x8_uzSeptini) > 0) {
  ...
  processingSeven(x8_uzSeptini, "7")
