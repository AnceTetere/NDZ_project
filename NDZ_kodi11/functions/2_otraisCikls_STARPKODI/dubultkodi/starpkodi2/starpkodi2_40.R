starpkodi2_40 <- function(y, t, prev, v) {
  ...
  if (t$zinkod[2] %in% c("21", "22", "23", "24", "25", "29")) {
      ...
      yt$dd <- as.numeric(sum(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "dd") - 1, 
                                  difftime(t$last_date[2], t$NDZ_sanemsanas_datums[2], units = "dd") + 1))
      } else if (t$period[1] == '______' && t$PS_code[1] == '_________' && t$NM_code[1] == '_________') {
        ...
        ZERO_minus(t %>% slice(2))
            ...
            if ((t$period[1] == '______' && t$PS_code[1] == '_________' && t$NM_code[1] == '_________') ||
                (t$period[1] == '______' && t$PS_code[1] == '_________' && t$NM_code[1] == '_________')) {
              yt$dd <- as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "dd")) - 1
              ZERO_plus(t %>% slice(2))
            } else {stop("Starpkodi2_40: Trūkst izstrādes koda.")}
               ...
               if(diff(t$NDZ_sanemsanas_datums) != 0) {
                 if(t$period[1] == "______" && t$PS_code[1] == '_________' && t$NM_code[1] == '_________') {
                   yt$dd <- as.numeric(sum(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "dd"),
                                               difftime(t$last_date[2], t$NDZ_sanemsanas_datums[2], units = "dd") + 1))
                 } else {stop("Starpkodi2_40: Trūkst izstrādes koda.")}
               } else {stop("Starpkodi2_40: Trūkst izstrādes koda.")}
  } else {stop("Starpkodi2_40: Trūkst izstrādes koda.")}
...
#  yt <- y[v,] 
#  yt$dd <- as.numeric(difftime(t$beidz_darbu[1], prev, units = "dd")) - 1 
...
#  dd2 <- as.numeric(difftime(t$last_date[2], t$sak_darbu[2], units = "dd")) + 1 
