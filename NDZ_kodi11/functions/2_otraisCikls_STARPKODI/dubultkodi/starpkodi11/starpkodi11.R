starpkodi11 <- function(y, t, prev, v) {
  ...
  if (t$zinkod[1] %in% c("11", "14", "16", "61")) {
               yt <- starpkodi11_11(y, t, prev, v)
  } else if (t$zinkod[1] %in% c("40", "50", "53", "91")) {
           if (t$zinkod[2] %in% c("40", "50", "53", "91")) {
             if (all(t$zinkod[c(6,7)] %in% c("40", "50", "53", "91")) && 
                       ...
                       if (t$zinkod[4] %in% c("41", "51", "54", "92")) {
                         if (t$zinkod[5] %in% c("41", "51", "54", "92")) {
                          if (t$zinkod[6] %in% c("40", "50", "53", "91")) {
                            if (all(t$zinkod[c(7,9)] %in% c("40", "50", "53", "91")) && 
                                all(t$zinkod[c(8,11)] %in% c("41", "51", "54", "92")) &&
                                ...
                                yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")) - 1,
                                   ...
                                   if (t$zinkod[6] %in% c("41", "51", "54", "92")) {
                                     if (t$zinkod[7] %in% c("40", "50", "53", "91")) {
                                      ...
                                      } else if (t$zinkod[9] %in% c("40", "50", "53", "91")) {
                                               if (t$zinkod[10] %in% c("21", "22", "23", "24", "25", "29")) {
                                                 if (t$zinkod[11] %in% c("41", "51", "54", "92")) {
                                                   if (diff(t$NDZ_sanemsanas_datums[10:11]) == 0 && all(diff(t$NDZ_sanemsanas_datums[1:10]) != 0)) {
                                                      #JO PIRMOREIZ
                                                      if ((t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________') ||
                                                          (t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________')) {
                                                           yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")) - 1,
