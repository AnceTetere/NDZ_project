      ...
      if (t$zinkod[5] %in% c("21", "22", "23", "24", "25", "29")) {
        if (t$zinkod[6] %in% c("41", "51", "54", "92")) {
          ...
          yt$dienas <- as.numeric(diff(t$NDZ_sanemsanas_datums[1:2]))
            ...
            #Indivīdu pieņem darbā - tas aiziet divos bezalgas atvaļinājumā.
                      yt$dienas <- as.numeric(sum(diff(t$NDZ_sanemsanas_datums[1:2]), diff(t$NDZ_sanemsanas_datums[3:4])))
               ...
               if (t$zinkod[6] %in% c("40", "50", "53", "91")) {
                ...
                } else if (diff(t$NDZ_sanemsanas_datums[1:2]) == 0 && all(diff(t$NDZ_sanemsanas_datums[2:6]) != 0)) {
                ...
                } else if (diff(t$NDZ_sanemsanas_datums[3:4]) == 0 && all(sapply(c(1,2,4,5), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
                           if ((t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________') ||
                               (t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________')) {
                                yt$dienas <- sum(sapply(c(1,3,5), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)])))
                           } else {stop("starpkodi6_11_50: Trūkst izstrādes koda. \n")}
