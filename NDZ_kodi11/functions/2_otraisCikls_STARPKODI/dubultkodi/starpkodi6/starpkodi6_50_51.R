        ...
        if (t$zinkod[5] %in% c("21", "22", "23", "24", "25", "29")) {
          if (t$zinkod[6] %in% c("41", "51", "54", "92")) {
            if (diff(t$NDZ_sanemsanas_datums[5:6]) == 0 && all(diff(t$NDZ_sanemsanas_datums[1:5]) != 0)) {
              ...
              if (t$period[1] == "202201" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
              ...
              if (t$period[1] == "202204" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
              ...
              yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")) - 1, 
                ...
                if (t$zinkod[6] %in% c("40", "50", "53", "91")) {
                  if (all(sapply(c(1,3), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
                      ...
                      if (t$period[1] == '202207' && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
                        t <- t[c(4,3,5,6),]; rownames(t) <- NULL
                        yt$dienas <- sum(sapply(c(1,3), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)])))
                      } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
                  } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
                } else if (t$zinkod[6] %in% c("21", "22", "23", "24", "25", "29")) {
                         if (all(diff(t$NDZ_sanemsanas_datums) != 0)) {
                             if (t$period[1] == '202208' && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
                                 yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")),
                                                  sapply(c(2,5), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]))) + 1
                             } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
                         } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
                } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
            } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
               ...
               if (t$zinkod[5] %in% c("21", "22", "23", "24", "25", "29")) {
                 if (t$zinkod[6] %in% c("41", "51", "54", "92")) {
                      if (t$period[1] == "202206" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
                          #JO PIRMOREIZ
                           yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")) - 1,
                        ...
                        if (t$zinkod[6] %in% c("21", "22", "23", "24", "25", "29")) {
                           if (t$period[1] == "202209" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
                               yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")) - 1,
                                                sapply(c(2,5), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]))) + 1
                            } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
                 } else {stop("starpkodi6_91: Trūkst izstrādes koda.")}
