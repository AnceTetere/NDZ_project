starpkodi5_11_50_51 <- function(y, t, prev, v) {
  ...
  if (t$zinkod[4] %in% c("21", "22", "23", "24", "25", "29")) {
        if (t$zinkod[5] %in% c("41", "51", "54", "92")) {
                       ...
                       } else if (all(diff(t$NDZ_sanemsanas_datums[1:4]) != 0) && 
           ...
           if (t$zinkod[5] %in% c("41", "51", "54", "92")) {
             if (diff(t$NDZ_sanemsanas_datums[3:4]) == 0 && all(sapply(c(1,2,4), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
                 if (t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________') {
                      ...
                      #Indivīds tiek pieņemts darbā un tanī pat dienā aiziet bezalgas atvaļinājumu.
                      #Atgriežas no tā un atkal aiziet bezalgas atvaļinājumā un atgriežas no tā.
                      yt$dienas <- sum(as.numeric(diff(t$NDZ_sanemsanas_datums[1:2])),
                                       as.numeric(diff(t$NDZ_sanemsanas_datums[3:4])),
