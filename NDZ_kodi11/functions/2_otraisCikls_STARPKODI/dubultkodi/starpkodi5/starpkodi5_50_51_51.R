starpkodi5_50_51_51 <- function(y, t, prev, v) {
  ...
  if (t$zinkod[4] %in% c("21", "22", "23", "24", "25", "29")) {
    if (t$zinkod[5] %in% c("40", "50", "53", "91")) {
      if (all(sapply(c(1,4), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]) == 0) & 
        ...
        } else {stop("starpkodi5_50_51_51: iztrūkst apstrādes koda.")}
      } else if (diff(t$NDZ_sanemsanas_datums[4:5]) == 0 && all(diff(t$NDZ_sanemsanas_datums[1:4]) != 0)) {
        if (t$period[1] == '______' && t$PS_code[1] ==  '______________' &&  t$NM_code[1] ==  '______________') {
          ...
          } else if (all(sapply(c(1,4), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) && 
