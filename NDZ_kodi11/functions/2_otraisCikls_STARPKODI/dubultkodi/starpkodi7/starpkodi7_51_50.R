                         ...
                         if (all(diff(t$NDZ_sanemsanas_datums[1:6]) != 0) &&diff(t$NDZ_sanemsanas_datums[6:7]) == 0) {
                             if (t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________') {
                                 ...
                                 yt$dienas <- sum(sapply(c(1,4), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)])),
                                    ...
                                    all(sapply(c(2,3,5), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
                                    if (t$period[1] == '______' && t$PS_code[1] ==  '______________' && t$NM_code[1] ==  '______________') {
                                      t <- t[c(2,1,3,4,5,7,6) ,]; rownames(t) <- NULL  
                                      yt$dienas <- sum(as.numeric(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days")) - 1,
                                                       sapply(c(4,6), function(i) diff(t$NDZ_sanemsanas_datums[i:(i+1)])))
                                    } else {stop("starpkodi7_51_50 iztrūkst apstrādes koda.")}
                         } else {stop("starpkodi7_51_50 iztrūkst apstrādes koda.")}
                      } else {stop("starpkodi7_51_50 iztrūkst apstrādes koda.")}
             } else {stop("starpkodi7_51_50 iztrūkst apstrādes koda.")}
