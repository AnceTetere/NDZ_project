...
if (t$zinkod[3] %in% c("21", "22", "23", "24", "25", "29")) {
      ...
      #  } else 
          if (all(diff(t$NDZ_sanemsanas_datums) == 0)) {
                   ...
                   #JO PIRMOREIZ
                   if ((t$period[1] == "202203" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') ||  #Labākais minējums; te var būt citādi.
                       (t$period[1] == "202208" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') ||
                       (t$period[1] == "202209" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________')) {
       ...
       if (diff(t$NDZ_sanemsanas_datums[2:3]) != 0) {
         ...
         if(t$period[1] == "202202" && t$PS_code[1] == '______________' && t$NM_code[1] == '______________') {
           yt$dienas <- as.numeric(diff(t$NDZ_sanemsanas_datums[2:3]))
           ZERO_plus(t %>% slice(2))
         } else {stop("Starpkodi3_51_50: Trūkst izstrādes koda.")}
       } else if (diff(t$NDZ_sanemsanas_datums[1:2]) != 0 && diff(t$NDZ_sanemsanas_datums[2:3]) == 0) {
