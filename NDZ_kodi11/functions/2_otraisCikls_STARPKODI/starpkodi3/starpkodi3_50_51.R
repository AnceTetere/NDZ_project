starpkodi3_50_51 <- function(y, t, prev, v) {
    ...
    if (t$zinkod[3] %in% c("21", "22", "23", "24", "25", "29")) {
               if (all(diff(t$NDZ_sanemsanas_datums) != 0)) {
                ...
                if (t$period[1] == '_______' && t$PS_code[1] %in% c('_______', 
                                                                    '_______', 
                                                                    '_______',
                                                                    '_______',
                                                                    '_______',
                                                                    '_______') && 
                    t$NM_code[1] %in% c('_______', 
                                        '_______', 
                                        '_______',
                                        '_______',
                                        '_______',
                   ...
                   yt <- y[v, ]
                   yt$dienas <- 0
                } else if (t$PS_code[1] %in% c('_______', '_______', '_______', '_______') && t$NM_code[1] == '_______') {
                  yt <- y[v, ]
                  yt$dienas <- 0
                } else if (t$period[1] == '_______' && t$PS_code[1] %in% c('_______', '_______') && t$NM_code[1] %in% c('_______', '_______')) {
                  yt <- y[v, ]
             ...
             if (diff(t$NDZ_sanemsanas_datums[2:3]) != 0 && diff(t$NDZ_sanemsanas_datums[1:2]) == 0) {
               if ((t$period[1] == "_______" && t$PS_code[1] == '_______' && t$NM_code[1] == '_______') ||
                   (t$period[1] == "_______" && t$PS_code[1] == '_______' && t$NM_code[1] == '_______')) {
                 yt <- y[v, ]
                 yt$dienas <- as.numeric(sum(difftime(t$NDZ_sanemsanas_datums[1], prev, units = "days") - 1,
