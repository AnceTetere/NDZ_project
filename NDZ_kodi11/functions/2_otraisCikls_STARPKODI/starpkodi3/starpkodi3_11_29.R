starpkodi3_11_29 <- function(y, t, prev, v) {
  ...
  if (all(diff(t$NDZ_sanemsanas_datums) == 0)) {
                  if (t$zinkod[3] %in% c("40", "50", "53", "91")) {
                    if (t$period[1] == "______" && t$PS_code[1] %in% c('__________', '______') && 
                        t$NM_code[1] %in% c('________', '________')) {
                      yt$dd <- as.numeric(diff(t$NDZ_sanemsanas_datums[1:2]))
                    } else if (t$period[1] == "______" && 
                               t$PS_code[1] %in% c('________', '________', '________', '________', '________', '________') && 
...
#    yt <- y2[v, ]
#    yt$dd <- as.numeric(difftime(t$beidz_darbu[3], t$sak_darbu[1], units = "days"))
