processingTwelve <- function(x, o) {
  ...
  for (r in seq(1, nrow(x), by = 12)) {
    ...
    } else if (sum(x12$start == "1") == 5) {
      if(all(x12$start[c(1, 3, 5, 7, 10)] == "1") && all(sapply(c(1:7,9,10), function(i) all(diff(x12$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) &&
         all(sapply(c(8,11), function(i) all(diff(x12$NDZ_sanemsanas_datums[i:(i+1)]) == 0)))) {
