...
for (r in seq(1, nrow(x), by = 4)) {
  ...
  if(!(doublesTest(1, x4) && doublesTest(3, x4))) {
  ...
  } else if (all(x4$sak_beidz[1:4] == c("2", "1", "2", "1"))) {
  ...
  } else if (all(x4$sak_beidz[1:4] == c("1", "2", "1", "1"))) {
    if (diff(x4$NDZ_sanemsanas_datums[1:2]) >= 0) {
  ...
  } else if (sum(x4$sak_beidz == "1") == 4 || sum(x4$sak_beidz == "2") == 4) {
      x4_uzVieniniekiem <- rbind(x4_uzVieniniekiem, codes_match(x4))
  } else if (all(x4$sak_beidz[1:4] == c("1", "1", "1", "2"))) {
      x4_trueDoubles <- rbind(x4_trueDoubles, x4[3:4, ])
  } else if (all(x4$sak_beidz[1:4] == c("1", "2", "2", "1"))) {
  ...
  } else if (all(x4$sak_beidz[1:4] == c("2", "1", "1", "1"))) {
    if (diff(x4$NDZ_sanemsanas_datums[1:2]) > 0) {
    ...
    if (diff(x4$NDZ_sanemsanas_datums[1:2]) == 0 && diff(x4$NDZ_sanemsanas_datums[c(2, 4)]) != 0) {
      x4_trueDoubles <- rbind(x4_trueDoubles, x4[1:2, ])
      x4_uzVieniniekiem <- rbind(x4_uzVieniniekiem, x4[4,])
    } else {
      stop("Šeit četrinieku izstrādes tabulas rindām ", r, " līdz ", r+3, " trūkst apstrādes koda.")
    }
  } else if (all(x4$sak_beidz[1:4] == c("2", "2", "1", "1"))) {
  ...
  } else if (all(x4$sak_beidz[1:4] == c("2", "2", "1", "2"))) { 
    if (diff(x4$NDZ_sanemsanas_datums[1:2]) > 0 && diff(x4$NDZ_sanemsanas_datums[2:3]) == 0) {
      x4_uzVieniniekiem <- rbind(x4_uzVieniniekiem, x4[1, ])
      x4_trueDoubles <- rbind(x4_trueDoubles, x4[c(2, 4), ])
    } else {
...
if(nrow(x4_trueDoubles) > 0) {
  ...
  cat("No četrinieku apstrādes tabula pārsūtīta uz processingTwoes, kas to tālāk nogādā uz tempNDZ datni.")
} else {
  cat("No četriniekiem pārsūtāmajā divnieku tabulā nebija nevienas rindas.")
}
rm(x4_trueDoubles)

#5 Atvasināto tabulu x4_uzTrijniekiem sūta caur processingThrees
