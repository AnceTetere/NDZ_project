adj50 <- function(NDZ, kods) {
  ...
  if (isTRUE(NDZ$period[1] == '______') && kods == "50") {
    NDZ <- NDZ %>% 
      dplyr::filter(!(PS_code ==  '______________' & NM_code ==  '______________' & zinkod == '50' & NDZ_sanemsanas_datums == as.Date('2021-01-04'))) #Kodam nav loģikas - tas maldina
   ...
   NDZ <- NDZ %>% 
     dplyr::filter(!(
       (PS_code ==  '______________' & NM_code ==  '______________' & zinkod == '51' & NDZ_sanemsanas_datums == '2021-10-01') | #Šim kodam nav loģikas.
       (PS_code ==  '______________' & NM_code ==  '______________' & zinkod == '51' & NDZ_sanemsanas_datums == '2021-10-01')   #Te tas pats.
         )) 
    ...
    NDZ <- changes_202111(NDZ, kods) 
      ...
      #Te dziļāka izpēte rāda, ka kods 11 ir jāņem te nost, jo indivīds uzsāk darbu vēlāk.
      NDZ <- NDZ %>% 
        dplyr::filter(!(
      ...
      NDZ <- NDZ %>% 
        dplyr::filter(!(PS_code ==  '______________' & NM_code ==  '______________' & NDZ_sanemsanas_datums == '2022-12-22' & zinkod == '25')) 
               ...
               NDZ <- NDZ %>% 
                        dplyr::filter(!(PS_code ==  '______________' & NM_code ==  '______________' & NDZ_sanemsanas_datums == '2023-01-31' & zinkod == '25')) #skat. notes_TODO p.6
