processingTens <- function(x, o) {
  ...
  for (r in seq(1, nrow(x), by = 10)) {
    ...
    x10 <- x[r:(r+9), ]
    x10 <- arrange(x10, PS_code, DN_code, NM_code, NDZ_sanemsanas_datums)
    ...
    if(sum(x10$sak_beidz == "1") == 5) {
      ...
      if (all(x10$sak_beidz[c(1, 2)] == "2") && all(x10$zinkod[c(1, 2)] == "26") &&
        ...
        x10_uzAstoniekiem <- rbind(x10_uzAstoniekiem, x10[3:10, ])
                 ...
                 x10$PS_code[1] == '________' & x10$NM_code[1] == '___________') {
        p <- x10[1:6, ]
...
if(nrow(x10_uzVieniniekiem) > 0) {
  x10_uzVieniniekiem <- arrange(x10_uzVieniniekiem, PS_code, NM_code, NDZ_sanemsanas_datums)
...
if(nrow(x10_uzDivniekiem) > 0) {
  x10_uzDivniekiem <- arrange(x10_uzDivniekiem, PS_code, NM_code, NDZ_sanemsanas_datums)
...
if(nrow(x10_uzSeptini) > 0) {
  x10_uzSeptini <- arrange(x10_uzSeptini, PS_code, NM_code, NDZ_sanemsanas_datums)
...
if(nrow(x10_uzAstoniekiem) > 0) {
  x10_uzAstoniekiem <- arrange(x10_uzAstoniekiem, PS_code,, NM_code, NDZ_sanemsanas_datums)
