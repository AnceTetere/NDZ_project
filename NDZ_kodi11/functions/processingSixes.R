processingSixes <- function(x, o) {
  ...
  for (r in seq(1, nrow(x), by = 6)) {
    ...
    x6 <- x[r:(r+5),]
    #TESTĒŠANAI: x[x$PS_codes == x$PS_codes[r], ]
    ...
    if (sum(x6$start == "1") == 6) {
      ...
      stop(
        cat(
          "Sešinieku tabula nepārdalījās.\n",
          "Nav apstrādes koda sešinieku apakštabulai!\n",
          "Problēmu skatīt rindā",
          r,
          ", lietojot", paste0("x[x$PS_codes == x$PS_codes[",
          r,
          "], ]"), "\n"
        )
      )
  ...
  cat("SKAIDROJUMS: Sešinieku tabula sadalīta tabulās x6_uzVieniniekiem:", nrow(x6_uzVieniniekiem), 
      "rindas; \n x6_uzDivniekiem: ", nrow(x6_uzDivniekiem), "rindas; \n x6_uzTris: ", nrow(x6_uzTris), 
      "rindas, x6_uzCetri: ", nrow(x6_uzCetri), "rindas, un \n x6_uzPieciniekiem: ", nrow(x6_uzPieciniekiem), "rindas.\n
    Tabulu x6_uzVieniniekiem tālāk pārstrādā caur processingOnes().\n
    Tabulu x6_uzDivniekiem tālāk pārstrādā caur processingTwoes().\n 
    Tabulu x6_uzCetri tālāk pārstrādā caur processingFours().\n 
    Tabulu x6_uzPieciniekiem pārstrādā caur processingFives().\n 
  ...
  if(nrow(x6_uzVieniniekiem) > 0) {
    x6_uzVieniniekiem <- x6_uzVieniniekiem[order(x6_uzVieniniekiem$PS_codes, x6_uzVieniniekiem$NM_codes, x6_uzVieniniekiem$NDZ_sanemsanas_datums), ]
  ...
  if(nrow(x6_uzDivniekiem) > 0) {
    x6_uzDivniekiem <- x6_uzDivniekiem[order(x6_uzDivniekiem$PS_codes, x6_uzDivniekiem$NM_codes, x6_uzDivniekiem$NDZ_sanemsanas_datums), ]
    ...
    cat("Atvasinātā tabula x6_uzDivniekiem no sešinieku pārsūtīta apstrādei caur processingTwoes, 
      ...
      uz oriģinālo indivīdu mēnesī definētu kā: period == PS_codes == DN_codes == NM_codes.\n")
  } else {
  ...
  if(nrow(x6_uzTris) > 0) {
    x6_uzTris <- x6_uzTris[order(x6_uzTris$PS_codes, x6_uzTris$NM_codes, x6_uzTris$NDZ_sanemsanas_datums), ]
  ...
  if(nrow(x6_uzCetri) > 0) {
    x6_uzCetri <- x6_uzCetri[order(x6_uzCetri$PS_codes, x6_uzCetri$NM_codes, x6_uzCetri$NDZ_sanemsanas_datums), ]
  ...
  if(nrow(x6_uzPieciniekiem) > 0) {
    x6_uzPieciniekiem <- x6_uzPieciniekiem[order(x6_uzPieciniekiem$PS_codes, x6_uzPieciniekiem$NM_codes, x6_uzPieciniekiem$NDZ_sanemsanas_datums), ]
    processingFives(x6_uzPieciniekiem, o)
    cat("Atvasinātā tabula x6_uzPieciniekiem no sešinieku pārsūtīta apstrādei caur processingTwoes, 
       un, pirms nolikšanas izstrādes tabulā temp_NDZ, dienas tiks sasummētas 
      uz oriģinālo indivīdu mēnesī definētu kā: period == PS_codes == DN_codes == NM_codes.\n")
  } else {
