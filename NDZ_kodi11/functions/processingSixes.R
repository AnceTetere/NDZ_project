      ...
      rm(x_vieninieki)
               ...
               if (all(x6$sak_beidz[c(1,3,5)] == "1")) {
                 ...
                 x6_uzDivniekiem <- rbind(x6_uzDivniekiem, x6[2:5, ])
                         ...
                         diff(x6$NDZ_sanemsanas_datums[3:4]) != 0 && 
                         all(sapply(seq(1,6, by=3), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+2)]) == 0))) {
                 x6_uzDivniekiem <- rbind(x6_uzDivniekiem, x6[c(1,4), ])
               } else if (all(x6$sak_beidz == c("2", "1", "2", "1", "2", "1")) && 
                         all(sapply(seq(1,4,by=2), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
                         diff(x6$NDZ_sanemsanas_datums[5:6]) != 0 &&
                 ...
                 x6_uzPieciniekiem <- rbind(x6_uzPieciniekiem, x6[-1, ])
                          ...
                          all(sapply(seq(2,4, by=2), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
                          all(sapply(seq(1,5, by=2), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
                 x6_uzPieciniekiem <- rbind(x6_uzPieciniekiem, x6[-1, ])
               } else if (all(x6$sak_beidz == c("1","2","1","2","2","1")) && diff(x6$NDZ_sanemsanas_datums) != 0) {
                 x6_uzCetri <- rbind(x6_uzCetri, x6[c(1,2,3,5), ])
                 x6_uzVieniniekiem <- rbind(x6_uzVieniniekiem, x6[6,])
               } else if (all(x6$sak_beidz == c("1", "2", "1", "1", "2", "2")) && 
                          all(sapply(c(1,2,3,5), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) != 0)) &&
                          diff(x6$NDZ_sanemsanas_datums[4:5]) == 0) {
                           x6_uzCetri <- rbind(x6_uzCetri, x6[c(1,2,3,5), ])
                           x6_uzDivniekiem <- rbind(x6_uzDivniekiem, x6[c(4,6),])
               ...
               if(all(x6$sak_beidz == c("1","1","2","1","2","1")) && 
                 ...
                 x6_uzVieniniekiem <- rbind(x6_uzVieniniekiem, x6[6, ])
               } else if(all(x6$sak_beidz == c("1","1","2","1","1","2"))) {
                         if (all(sapply(c(2,5), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
                             all(sapply(c(1,3,4), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
                           if (x6$period[1] == '___________' && x6$PS_code[1] == '___________' && x6$NM_code[1] == "___________") {
                             x6_uzCetri <- rbind(x6_uzCetri, x6[c(1,3,5,6), ])
                           } else {stop("processingSixes trūkst apstrādes koda. \n")}
                         } else {stop("processingSixes trūkst apstrādes koda. \n")}
                 ...
                 x6_uzVieniniekiem <- rbind(x6_uzVieniniekiem, x6[6, ])
                      ...
                      if(all(sapply(c(1,4), function(i) diff(x6$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
                        ...
                        if(x6$PS_code[1] == '___________' && x6$NM_code[1] == '___________') {
