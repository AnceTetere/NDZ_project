processingFifteen <- function(x, o) {
  ...
  x15_uzVieniniekiem <- data.frame(); x15_uzDivniekiem <- data.frame(); x15_uzSesi <- data.frame(); x15_uzCetrpadsmit <- data.frame()
  check_rows <- 0
...
if (nrow(x15_uzVieniniekiem) > 0) {
    ...
    x15_uzVieniniekiem %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingOnes(o) %>% sendTo_tempNDZ()
...
if (nrow(x15_uzSesi) > 0) {
    ...
    x15_uzSesi %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingSixes(o)
...
if (nrow(x15_uzCetrpadsmit) > 0) {
    ...
    x15_uzCetrpadsmit %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingFourteen(o)
