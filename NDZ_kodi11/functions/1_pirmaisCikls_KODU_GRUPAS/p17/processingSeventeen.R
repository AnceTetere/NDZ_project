processingSeventeen <- function(x, o, kods) {
...
if (sum(x17$sak_beidz == "2") == 9) {
      ...
      } else if (x17$sak_beidz[17] == "1" && x17$sak_beidz[16] != "1") {
  ...
  if (nrow(x17_uzDivniekiem) > 0) {
    x17_uzDivniekiem %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingTwoes(o, kods)
  ...
  if (nrow(x17_uzTrīspadsmit) > 0) {
   x17_uzTrīspadsmit %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingThirteen(o, kods)
