processingTwoes <- function(x, o, kods) {
  ...
  if(nrow(x2_uzVieniniekiem) > 0) {
    cat(sendTo_tempNDZ(processingOnes(x2_uzVieniniekiem, o), o))
  } else {
...
#3.2 Atvasinātos un pārbaudītos pārus sūta apstrādei caur funkciju processingDoubles()
  cat(sendTo_tempNDZ(processingDoubles(x2_trueDoubles, o, kods), o))
    
  } else {cat("Tabula x2_trueDoubles ir tukša")}
  rm(x2_trueDoubles)
}
