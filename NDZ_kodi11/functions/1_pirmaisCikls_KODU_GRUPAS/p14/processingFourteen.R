processingFourteen <- function(x, o, kods) {
  ...
  x14_uzVieniniekiem <- data.frame(); x14_uzDivi <- data.frame()
  x14_uzDivpadsmit <- data.frame(); x14_uzTrispadsmit <- data.frame()
  ...
  if (nrow(x14_uzVieniniekiem) + nrow(x14_uzDivi) + nrow(x14_uzDivpadsmit) + nrow(x14_uzTrispadsmit) == nrow(x)) {
    ...
    stop("ERROR: Apakštabulu rindu summa NESAKRĪT ar izejošo 14-nieku tabulu.\n")
  }
