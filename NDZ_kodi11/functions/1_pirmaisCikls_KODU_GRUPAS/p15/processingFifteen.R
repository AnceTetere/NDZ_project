processingFifteen <- function(x, o, kods) {
  ...
  x15_uzVieniniekiem <- data.frame(); x15_uzDivniekiem <- data.frame(); x15_uzSesi <- data.frame()
  x15_uz13 <- data.frame(); x15_uzCetrpadsmit <- data.frame()
  check_rows <- 0
  ...
  for (r in seq(1, nrow(x), by = 15)) {
    ...
    if (all(x15$sak_beidz[1:4] == c("2", "1", "2", "1"))) {
            if (all(sapply(c(1,3), function(i) diff(x15$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) && diff(x15$NDZ_sanemsanas_datums[2:3]) != 0) {
              x15_uzDivniekiem <- rbind(x15_uzDivniekiem, x15[c(2,1), ])
            ...
            if (all(diff(x15$NDZ_sanemsanas_datums) != 0)) {
              x15_uzDivniekiem <- rbind(x15_uzDivniekiem, x15[1:2, ])
              x15_uz13 <- rbind(x15_uz13, x15[-(1:2), ])
            } else {stop("15-niekos trūkst izstrādes kods.")}
...
if (nrow(x15_uzVieniniekiem) > 0) {
    x15_uzVieniniekiem %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingOnes(o) %>% sendTo_tempNDZ(o)
...
#4 Apakštabulu x15_uzCetrpadsmit sūta caur processingFourteen().
if (nrow(x15_uzCetrpadsmit) > 0) {
    x15_uzCetrpadsmit %>% arrange(PS_code, NM_code, NDZ_sanemsanas_datums) %>% processingFourteen(o, kods)
