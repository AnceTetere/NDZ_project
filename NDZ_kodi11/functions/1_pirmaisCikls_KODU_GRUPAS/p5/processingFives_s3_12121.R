...
if (diff(x5s3s$NDZ_sanemsanas_datums[3:4]) == 0 &&
    ...
    if((x5s3s$PS_code[1] %in% c('___________', '___________', '___________', 
                                '___________', '___________') && x5s3s$NM_code[1] %in% c('__________')) ||
       (x5s3s$PS_code[1] == '___________' && x5s3s$NM_code[1] == '__________') ||
       (x5s3s$PS_code[1] == '___________' && x5s3s$NM_code[1] == '__________') ||
       (x5s3s$period[1] == '_____' && x5s3s$PS_code[1] == '___________' && x5s3s$NM_code[1] == '__________') ||
       (x5s3s$period[1] == '_____' && x5s3s$PS_code[1] == '__________7' && x5s3s$NM_code[1] == '__________')) {
          ...
          x5s3s_uzDivniekiem <- rbind(x5s3s_uzDivniekiem, x5s3s[1:4,])
           ...
           all(sapply(c(2,4), function(i) diff(x5s3s$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
              if ((x5s3s$PS_code[1] == '__________' && x5s3s$NM_code[1] == '__________') ||
                  (x5s3s$PS_code[1] == '__________' && x5s3s$NM_code[1] == '__________') ||
                  (x5s3s$PS_code[1] == '__________' && x5s3s$NM_code[1] == '__________') ||
                  (x5s3s$PS_code[1] == '__________' && x5s3s$NM_code[1] == '__________') ||
                  (x5s3s$period[1] == "_____" && x5s3s$PS_code[1] == '__________' && x5s3s$NM_code[1] == '__________')) {
