processingSixes_s3 <- function(x6s3, o, kods) {
  ...
  if (all(x6s3$sak_beidz[c(1,3,5)] == "1")) {
    ...
    x6s3_uzDivniekiem <- rbind(x6s3_uzDivniekiem, x6s3[c(1,4), ])
             ...
             diff(x6s3$NDZ_sanemsanas_datums[5:6]) != 0 &&
             x6s3$PS_code[1] == "_____" && x6s3$NM_code[1] == "_____") {
    ...
    if (kods %in% c("40", "50", "53") && o == "6") {ZERO_minus(x6s3 %>% slice(1)); ZERO_plus(x6s3 %>% slice(6))}
            ...
            if ((x6s3$period[1] == '_____' && x6s3$PS_code[1] == '_____' && x6s3$NM_code[1] == '_____') ||
                (x6s3$period[1] == '_____' && x6s3$PS_code[1] == '_____' && x6s3$NM_code[1] == '_____')) {
