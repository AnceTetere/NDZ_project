processingSixes_s3 <- function(x6s3, o, kods) {
  ...
  if (all(x6s3$sak_beidz[c(1,3,5)] == "1")) {
    ...
    if (kods %in% c("40", "50", "53") && o == "6") {ZERO_minus(x6s3 %>% slice(1)); ZERO_plus(x6s3 %>% slice(6))}
  } else if (all(x6s3$sak_beidz[c(1,2,5)] == "1") && 
            ...
            if (kods %in% c("40", "50", "53") && o == "6") {ZERO_minus(x6s3 %>% slice(1)); ZERO_plus(x6s3 %>% slice(6))}
  } else if(all(x6s3$sak_beidz[c(3,4,6)] == "1")) {
          if (diff(x6s3$NDZ_sanemsanas_datums[2:3]) == 0 && all(diff(x6s3$NDZ_sanemsanas_datums[3:6]) != 0)) {
            x6s3_uzVieniniekiem <- x6s3[c(1,6), ]; x6s3_uzDivniekiem <- x6s3[2:5, ]
          } else if (all(diff(x6s3$NDZ_sanemsanas_datums) != 0)) {
                if (x6s3$period[1] == "_____" && x6s3$PS_code[1] == "__________" && x6s3$NM_code[1] == "__________") {
                x6s3_uzVieniniekiem <- x6s3[c(2,6), ]; x6s3_uzDivniekiem <- x6s3[4:5, ]
             ...
             diff(x6s3$NDZ_sanemsanas_datums[5:6]) != 0 &&
             x6s3$PS_code[1] == "__________" && x6s3$NM_code[1] == "__________") {
             ...
             all(sapply(c(1,3,5), function(i) diff(x6s3$NDZ_sanemsanas_datums[i:(i+1)]) != 0)) &&
             all(sapply(c(2,4), function(i) diff(x6s3$NDZ_sanemsanas_datums[i:(i+1)]) == 0))) {
              if ((x6s3$period[1] == '_____' && x6s3$PS_code[1] == '__________' && x6s3$NM_code[1] == '__________') ||
                  (x6s3$period[1] == '_____' && x6s3$PS_code[1] == '__________' && x6s3$NM_code[1] == '__________')) {
                    x6s3 <- x6s3[c(1,3,2,5,4,6),]; x6s3_uzDivniekiem <- x6s3
                    if (kods %in% c("40", "50", "53") && o == "6") {ZERO_plus(x6s3 %>% slice(6))}
            } else {stop("processingSixes_s3 trūkst apstrādes koda. \n")}
  } else if (all(x6s3$sak_beidz == c("1", "2", "2", "1", "2", "1")) && 
             all(diff(x6s3$NDZ_sanemsanas_datums) != 0)) {
            if (x6s3$period[1] == '_____' && x6s3$PS_code[1] == '__________' && x6s3$NM_code[1] == '__________') {
              x6s3_uzVieniniekiem <- rbind(x6s3_uzVieniniekiem, x6s3[6,])
              x6s3_uzDivniekiem <- rbind(x6s3_uzDivniekiem, x6s3[c(1,3,4,5),])
              if (kods %in% c("40", "50", "53") && o == "6") {ZERO_minus(x6s3 %>% slice(1))}
            } else {stop("processingSixes_s3 trūkst apstrādes koda. \n")}
  } else if (all(x6s3$sak_beidz == c("2", "2", "1", "1", "1", "2")) &&
             all(sapply(c(1,3), function(i) diff(x6s3$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
             all(sapply(c(2,4,5), function(i) diff(x6s3$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
            if (x6s3$period[1] == '_____' && x6s3$PS_code[1] == '__________' && x6s3$NM_code[1] == '__________') {
            x6s3_uzVieniniekiem <- x6s3[1,]; x6s3_uzDivniekiem <- x6s3[5:6,]
            if (kods %in% c("40", "50", "53") && o == "6") {ZERO_plus(x6s3 %>% slice(6))}
            } else {stop("processingSixes_s3 trūkst apstrādes koda. \n")}
  } else {stop("processingSixes_s3 trūkst apstrādes koda. \n")}
