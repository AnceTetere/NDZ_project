processingSeven_s4_1467 <- function(a, o, kods) {
  ...
  if (all(diff(a$NDZ_sanemsanas_datums) != 0)) {
            ...
            if (a$period[1] == "202203" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') {
             ...
             all(sapply(c(2,4,6), function(i) diff(a$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
               ...
               if ((a$period[1] == "202204" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') ||
                   (a$period[1] == "202204" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') ||
                   (a$period[1] == "202205" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________')) {
                     a <- a[c(1,2,4,3,6,5,7),]; a6 <- a[1:6, ]; a1 <- a[7, ]
                     if (kods %in% c("40", "50", "53") && o == "7") {ZERO_minus(a %>% slice(1))}
               } else {stop("processingSeven_s4 trūkst izstrādes koda.\n")}
  } else if (all(sapply(c(1,2,4,6), function(i) diff(a$NDZ_sanemsanas_datums[i:(i+1)]) != 0)) &&
             all(sapply(c(3,5), function(i) diff(a$NDZ_sanemsanas_datums[i:(i+1)]) == 0))) {
              #PAGAIDĀM VISS KĀRTĪBĀ.
               if ((a$period[1] == "202204" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') ||
                   (a$period[1] == "202205" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') ||
                   (a$period[1] == "202207" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') ||
                   (a$period[1] == "202209" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________')) {
                    a <- a[c(1,2,4,3,6,5,7),]; a6 <- a[1:6, ]; a1 <- a[7, ]
                    if (kods %in% c("40", "50", "53") && o == "7") {ZERO_minus(a %>% slice(1))}
              } else {stop("processingSeven_s4 trūkst izstrādes koda.\n")}
  } else if (diff(a$NDZ_sanemsanas_datums[1:2]) == 0 && all(diff(a$NDZ_sanemsanas_datums[2:7]) != 0)) {
            #PAGAIDĀM VISS KĀRTĪBĀ.
            if (a$period[1] == "202209" && a$PS_code[1] == '______________' && a$NM_code[1] == '______________') {
                a6 <- a[1:6, ]; a1 <- a[7, ]
                if (kods %in% c("40", "50", "53") && o == "7") {ZERO_minus(a %>% slice(1))}
            } else {stop("processingSeven_s4 trūkst izstrādes koda.\n")}
  } else {stop("processingSeven_s4 trūkst izstrādes koda.\n")}
