 processingSeven_s4_1346 <- function(a, o, kods) {
  ...
  } else if (all(sapply(c(4,6), function(i) diff(a$NDZ_sanemsanas_datums[i:(i+1)]) == 0)) &&
             all(sapply(c(1,2,3,5), function(i) diff(a$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
              if ((a$period[1] == '______' && a$PS_code[1] ==  '______________' && aNM_code ==  '______________') ||
                  (a$period[1] == '______' && a$PS_code[1] ==  '______________' && aNM_code ==  '______________')) {
                   a6 <- a[c(1,2,4,5,6,7), ]
                   if (kods %in% c("40", "50", "53") && o == "7") {ZERO_minus(a %>% slice(1)); ZERO_plus(a %>% slice(7))}
