...
x7b4_uzVieniniekiem <- data.frame()
x7b4_uzDivniekiem <- data.frame()
x7b4_uzTrijniekiem <- data.frame()
...
x7b4_uzSesiniekiem <- data.frame()
  ...
  if ((all(x7b4$sak_beidz[1:2] == c("2", "1")) && diff(x7b4$NDZ_sanemsanas_datums[1:2]) != 0) || 
    ...
    x7b4_uzSesiniekiem <- rbind(x7b4_uzSesiniekiem, x7b4[-1, ])
             ...
             x7b4$PS_code[1] == '_________' && x7b4$NM_code[1] == '_________') {
    x7b4_uzVieniniekiem <- rbind(x7b4_uzVieniniekiem, x7b4[1, ])
    x7b4_uzSesiniekiem <- rbind(x7b4_uzSesiniekiem, x7b4[-1, ])
    ...
    if (all(diff(x7b4$NDZ_sanemsanas_datums) != 0)) {
      if(x7b4$period[1] == "_________" && x7b4$PS_code[1] == "_________" && x7b4$NM_code[1] == '_________') {
