processingNines <- function(x, o, kods) {
  ...
  for(r in seq(1, nrow(x), by = 9)) {
    ...
    x9 <- x[r:(r+8),] %>% arrange(PS_code, DN_code, NM_code, NDZ_sanemsanas_datums)
     ...
     if (all(x9$sak_beidz == c("2", "1", "2", "1", "1", "2", "2", "1", "1")) && 
      ...
      x9_uzVieniniekiem <- rbind(x9_uzVieniniekiem, x9[9, ])
      x9_uzAstoniekiem <- rbind(x9_uzAstoniekiem, x9[1:8, ])
      if (kods %in% c("40", "50", "53") && o == "9") {ZERO_minus(x9 %>% slice(1))}
      ...
      if (kods %in% c("40", "50", "53") && o == "9") {ZERO_plus(x9 %>% slice(9))}
    } else if (all(x9$sak_beidz == c("1", "2", "1", "1", "2", "1", "2", "1", "2"))) {
          if (diff(x9$NDZ_sanemsanas_datums[2:3]) == 0) {
            if (x9$period[1] == "______" && x9$PS_code[1] == "______" && x9$NM_code[1] == "______") {
              x9_uzAstoniekiem <- rbind(x9_uzAstoniekiem, x9[c(1:2, 4:9), ])
              if (kods %in% c("40", "50", "53") && o == "9") {ZERO_minus(x9 %>% slice(1)); ZERO_plus(x9 %>% slice(9))}
    ...
    } else if (all(x9$sak_beidz[1:4] == c("2", "2", "1", "1")) && all(x9$sak_beidz[8:9] == c("2", "1"))) {
            if (diff(x9$NDZ_sanemsanas_datums[2:3]) == 0 && all(sapply(c(1,3), function(i) diff(x9$NDZ_sanemsanas_datums[i:(i+1)]) != 0))) {
              if (x9$period[1] == "______" && x9$PS_code[1] == "______" && x9$NM_code[1] == "______") {
              x9_uzVieniniekiem <- rbind(x9_uzVieniniekiem, x9[1, ])
              x9_uzAstoniekiem <- rbind(x9_uzSesi, x9[c(3,2,4,5,6,7,9,8), ])
              } else {stop("processingNines() iztrūkst apstrādes kods. \n")}
            } else {stop("processingNines() iztrūkst apstrādes kods. \n")}
